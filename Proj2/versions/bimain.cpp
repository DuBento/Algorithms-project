...
*/
    ...
    void auxFillEdges(Vertex*** map, Type type, vector<Vertex*> *v) {
    ...
    virtual void fillEdgesIn() {        // fill edges for vertices of type in
        // cout << _xPos << ";" << _yPos << "\n";
        auxFillEdges(map_out, SHOP, &_parents);
        _children.push_back(map_out[_xPos][_yPos]);
    }

    virtual void fillEdgesOut() {       // fill edges for vertices of type out
        // cout << _xPos << ";" << _yPos << "\n";
        _parents.push_back(map_in[_xPos][_yPos]);
        auxFillEdges(map_in, CLIENT, &_children);
    }

    void swapPtoC(Vertex *v) {

    }

    void swapCtoP(Vertex *v) 
...
void parseValues() {
    ...
    for (int i = 0; i < nShops; i++) {
        ...
        //add to target
        t->_parents.push_back(map_out[x][y]); 
    }
    ...
    for (int i = 0; i < nClients; i++) {
        ...
        //add to source
        s->_children.push_back(map_in[x][y]); 
...
vector<Vertex*> findPath() {
    ...
    t->setNotVisited();
    bool endFound = false;
    vector<Vertex*> path;   // saves augmentation path from s to t
...
void printParents(Vertex*** map){
    cout << "PARENTS" << "\n";
    for(int i = 0; i<nAvenues; i++)
        for(int j=0; j<nStreets; j++){
            cout << "---------- " << i << ";" << j << "\tsize" << map[i][j]->_parents.size() <<'\n';
            unsigned sk = map[i][j]->_parents.size();
            for(unsigned int k =0 ; k < sk; k++)
                cout << i+1 << ";" << j+1 << "\t" << *(map[i][j]->_parents[k]) << '\n';
        }
}
